...
class SqlAlchemyAuditIntakeRepository:
    ...
    def complete_upload_validation(
        ...
        *,
        ...
        with self._sessions.begin() as session:
            model = get_upload_session(session, session_id)
            require_upload_state(
                model,
                {UploadSessionState.UPLOADING, UploadSessionState.VALIDATING},
            )
            validations = {entry.file_id: entry for entry in files}
            for file in model.files:
                entry = validations.get(file.file_id)
                if entry is None:
                    raise AuditStateError(f"Upload file {file.file_id} is missing validation.")
            ...
            model.state = UploadSessionState.READY_TO_CREATE.value if valid else UploadSessionState.INVALID.value
        return to_upload_session_record(model)
        ...
        *,
        ...
        try:
            with self._sessions.begin() as session:
                ...
                if session.scalar(select(ProjectModel.project_id).where(ProjectModel.name == name)) is not None:
                ...
                for file in upload.files:
                    if not file.content_hash or not file.logical_role:
                        raise AuditStateError("All uploaded files must be validated before promotion.")
                    session.add(SourceDocumentModel(
                        document_id=str(uuid4()),
                        snapshot_id=source_snapshot_id,
                        relative_path=file.relative_path,
                        logical_role=file.logical_role,
                        original_object_key=file.staging_object_key,
            ...
            raise
        return to_project_record(project, snapshot, 2), to_version_record(version)
